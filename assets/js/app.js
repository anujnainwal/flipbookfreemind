...
(function (window, $, Backbone) {
  ...
  var TableContentsView = SingleView.extend({
    ...
    initialize: function () {
      $("#ui-icon-table-contents").on("click", $.proxy(this, "show"));

      //  $('#ui-icon-table-content1').on('click', $.proxy(this, 'hide'));
    },
    ...
    _fade: function (event) {
      if (!event.donotFade) {
        ...
        if (event.pageY > $("#viewer").height() - 20) {
          if (this.$el.hasClass("extend-ui-options")) {
            this.$el.removeClass("extend-ui-options");
          }
        }
        ...
        this._fadeTimer = setTimeout(function () {
          if (!MiniaturesView.getInstance().isOpened()) {
            ...
            that.$el.addClass("extend-ui-options");
